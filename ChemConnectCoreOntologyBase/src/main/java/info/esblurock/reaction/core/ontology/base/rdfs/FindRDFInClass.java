...
public class FindRDFInClass {
	...
	public static ListOfRDFInformation find(String classname) {
		...
		List<String> elements = OntologyUtilityRoutines.exactlyOnePropertyMultiple(classname,
				OntologyObjectLabels.mappingRelation);
	 ...
	 * 
	 *         If there are multiple values of a subject or object, the same number
	 *         extra RDFInformation are created.
	 * 
	 *         The first loop is over the set of RDFInformation objects.
	 *         For each RDFInformation loop through the object classes and the
	 *         subject classes.
	 *         The RDFInformationList is initiated by the current RDFInformation.
	 *         If either the subject or object has multiple n values, then n
	 *         RDFInformations are created.
	 ...
	 *         A copy of the current is made and the each new value is added to each
	 *         new RDFInformaion
	 *         If there are n multiple values,
	 */
	public static ListOfRDFInformation fillWithValues(ListOfRDFInformation rdfs, JsonObject obj, String id,
			boolean subject) {
		...
		if (arr.size() == 0) {
			// System.out.println("Ontology Error: fillWithValues: No values for id: " + id
			// + "RDF: " + rdfs.getList().get(0).getPredicateClass());
			// System.out.println("Ontology Error: fillWithValues: Object: " +
			// JsonObjectUtilities.toString(obj));
			// System.out.println("Ontology Error: fillWithValues: RDFInformation: " +
			// JsonObjectUtilities.toString(rdfs.getList().get(0).toJsonObject()));
			finalrdfsInformation = null;
		}
		List<RDFInformation> rdflist = rdfs.getList();
		for (RDFInformation rdf : rdflist) {
			// if
			// (rdf.getClassname().equals("dataset:JThermodynamicsSymmetryStructureDefinitionDataSet"))
			// {
			// System.out.println("fillWithValues: "+ id + subject + " " +
			// JsonObjectUtilities.toString(arr));
			// }
			for (JsonElement val : arr) {
				...
				// (rdf.getClassname().equals("dataset:JThermodynamicsSymmetryStructureDefinitionDataSet"))
				// {
				// System.out.println("fillWithValues:
				// dataset:JThermodynamicsSymmetryStructureDefinitionDataSet: '" + valueString +
				// "'");
				// }
	 ...
	 * 
	 *                  Within the RDF class name, find the list of entities
	 *                  (objects) and subjects class names.
	 *                  Loop over these two lists of classes and convert them to two
	 *                  lists of identities.
	 *                  Create an RDFInformation with the list of subject
	 *                  identities, the list of object identities,
	 * 
	 */
	private static void createBaseRDFInformation(String classname, String rdfclass, ListOfRDFInformation lst) {
		List<String> entities = OntologyUtilityRoutines.exactlyOnePropertyMultiple(rdfclass,
				OntologyObjectLabels.entity);
		...
		List<String> subjects = OntologyUtilityRoutines.exactlyOnePropertyMultiple(rdfclass,
				OntologyObjectLabels.subject);
