...
public class DataStoreController {
    ...
    public String getUITemplate(@RequestBody Map<String, Object> params) {
        try {
            ...
            JsonObject response = StandardResponse.standardServiceResponse(doc, "Template generated successfully",
                    template);
            ...
            errorResponse.addProperty("dataset:serviceresponsemessage",
                    "Template-side exception (" + e.getClass().getSimpleName() + "): " + e.getMessage());
        ...
        try {
            ...
            if (shouldWrite) {
                ...
                if (firestoreSuccess) {
                    ...
                    message += (opName != null) ? " Discovery Engine sync triggered (Op: " + opName + ")."
                            : " but Discovery Engine sync FAILED to trigger (check logs).";
                } else {
    ...
    public String listTermsByDatatype(@RequestBody Map<String, Object> params) {
        try {
            org.dom4j.Document doc = info.esblurock.reaction.core.MessageConstructor
                    .startDocument("ListDataStoreTerms");
            String datatype = (String) params.get("datatype");
            DictionaryTermSet termSet = dictionarySearchService.getTermsByDatatype(datatype);
            JsonArray termsArray = termSet.toJsonArray();
            JsonObject response = info.esblurock.reaction.core.StandardResponse.standardServiceResponse(doc,
                    "Fetched " + termSet.getTerms().size() + " terms.", termsArray);
        ...
        try {
            ...
            params.forEach((key, value) -> {
                if (value instanceof String)
                    input.addProperty(key, (String) value);
                else if (value instanceof Number)
                    input.addProperty(key, (Number) value);
                else if (value instanceof Boolean)
                    input.addProperty(key, (Boolean) value);
            });
