...
public class ExtractDescription {
    ...
    public DescriptionResult extract(String prompt, String datatype) throws IOException {
        ...
        try (SearchServiceClient client = SearchServiceClient.create(settings)) {
            ...
            // We use the provided datatype term set to provide relevant dictionary context
            // for the LLM.
            ...
            if (response.hasSummary() && response.getSummary().getSummaryText() != null
                    && !response.getSummary().getSummaryText().isEmpty()) {
                ...
                try {
                    ...
                    System.err.println("Error parsing summarization agent output: " + e.getMessage()
                            + " | Raw JSON was: " + cleanJson);
                }
            } else {
                System.out.println(
                        "No summary returned for ExtractDescription. Summary result: " + response.getSummary());
            }
        }
