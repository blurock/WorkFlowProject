...
@Component({
  selector: 'app-parameter-specification-temperature',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
    ...
    <div class="spec-container" [class.expanded]="expanded">
      ...
      <div class="details-section" *ngIf="expanded">
        <div *ngIf="loading" class="loading-template-msg">
          Loading Temperature specification template...
        </div>
        ...
        <div *ngIf="!loading">
          ...
          <div *ngIf="propertyKeys.length === 0" class="no-props-msg">
            No detailed properties available for this Temperature specification.
          </div>
        </div>
      </div>
    </div>
...
export class ParameterSpecificationTemperatureComponent extends BasePrimitiveComponent implements OnInit {
  ...
  override ngOnInit(): void {
    ...
    if (!hasProperties) {
      const cls = this.classname || this.structure?.classname || 'dataset:ParameterSpecificationTemperature';
      this.loading = true;
      this.ontologyService.getUITemplate(cls).subscribe({
        ...
        error: (err) => {
          console.error('[ParameterSpecificationTemperatureComponent] Failed to load template for class: ' + cls, err);
          this.loading = false;
          this.cdr.detectChanges();
        }
      });
  ...
  mapGenericToSpecific(): void {
    ...
    if (!this.value['dataset:parameterlabeltemperature']) {
      ...
      if (labelVal) {
        this.value['dataset:parameterlabeltemperature'] = labelVal;
      }
    }

    // 2. Units Mapping
    if (!this.value['dataset:temperatureunits']) {
      ...
      if (srcUnit) {
        ...
        const unitclass = typeof srcUnit === 'string' ? 'dataset:UnitClassTemperature' : (srcUnit['qudt:SystemOfQuantities'] || 'dataset:UnitClassTemperature');
        unitspec = this.translateQudtUnit(unitspec);
        this.value['dataset:temperatureunits'] = {
          'dataset:temperatureunitclass': unitclass,
          'dataset:temperatureunitspecification': unitspec
        };
      }
    } else {
      const specObj = this.value['dataset:temperatureunits'];
      if (specObj && specObj['dataset:temperatureunitspecification']) {
        specObj['dataset:temperatureunitspecification'] = this.translateQudtUnit(specObj['dataset:temperatureunitspecification']);
      }
    }

    // 3. Purpose/Concept Mapping
    if (!this.value['dataset:purposeconcepttemperature']) {
      this.value['dataset:purposeconcepttemperature'] = {
  ...
  getLabel(): string {
    if (this.value) {
      if (this.value['dataset:parameterlabeltemperature']) return this.value['dataset:parameterlabeltemperature'];
  ...
  getUnits(): string {
    const unitspec = this.value?.['dataset:temperatureunits'] || this.value;
    const unitObj = unitspec?.['dataset:temperatureunitspecification'] || unitspec?.['qudt:Unit'] || unitspec?.['dataset:units'];
    if (unitObj) {
  ...
  private cleanUnitString(unit: string): string {
    ...
    if (lower === 'kelvin' || lower === 'unitskelvin') return 'K';
    if (lower === 'celcius' || lower === 'unitscelcius') return '°C';
    if (lower === 'fahrenheit' || lower === 'unitsfahrenheit') return '°F';
    if (lower === 'rankine' || lower === 'unitsrankine') return 'R';
    if (lower === 'millidegreecelcius' || lower === 'unitsmillidegreecelcius') return 'm°C';
    return cleaned;
  }
  ...
  updateProperty(key: string, newValue: any): void {
    if (this.value) {
      ...
      } else if (key === 'dataset:temperatureunits') {
        const unitspec = newValue?.['dataset:temperatureunitspecification'];
        const unitclass = newValue?.['dataset:temperatureunitclass'];
