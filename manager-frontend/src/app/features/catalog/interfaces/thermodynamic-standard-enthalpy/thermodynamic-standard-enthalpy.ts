...
@Component({
  selector: 'app-thermodynamic-standard-enthalpy',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class ThermodynamicStandardEnthalpyComponent extends BasePrimitiveComponent implements OnInit {
  ...
  override ngOnInit(): void {
    ...
    if (!hasProperties) {
      const cls = this.classname || this.structure?.classname || 'dataset:ThermodynamicStandardEnthalpy';
      this.loading = true;
      this.ontologyService.getUITemplate(cls).subscribe({
        ...
        error: (err) => {
          console.error('[ThermodynamicStandardEnthalpyComponent] Failed to load template: ' + cls, err);
          this.loading = false;
          this.cdr.detectChanges();
        }
      });
  ...
  getUnits(): string {
    const spec = this.value?.['qb:ComponentSpecification'];
    const unitObj = spec?.['qudt:Unit'] || spec?.['dataset:units'] || spec?.['dataset:molarenthalpyunitspecification'];
    if (unitObj) {
  ...
  hasSpec(): boolean {
    return !!(this.resolvedStructure?.properties?.['dataset:paramspecenthalpy'] || 
              this.resolvedStructure?.properties?.['qb:ComponentSpecification'] || 
              this.value?.['dataset:paramspecenthalpy'] || 
              this.value?.['qb:ComponentSpecification']);
  }

  getSpecStructure(): OntologyStructure {
    if (this.resolvedStructure?.properties?.['dataset:paramspecenthalpy']) {
      return this.resolvedStructure.properties['dataset:paramspecenthalpy'];
    }
    ...
    return {
      identifier: 'dataset:paramspecenthalpy',
      classname: 'dataset:ParameterSpecificationEnthalpy',
  ...
  updateSpec(newSpec: any): void {
    if (this.value) {
      ...
      if ('dataset:paramspecenthalpy' in updatedValue || !('qb:ComponentSpecification' in updatedValue)) {
        updatedValue['dataset:paramspecenthalpy'] = newSpec;
      } else {
