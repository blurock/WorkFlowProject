...
@Component({
  selector: 'app-parameter-specification-enthalpy',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
    ...
    <div class="spec-container" [class.expanded]="expanded">
      ...
      <div class="details-section" *ngIf="expanded">
        <div *ngIf="loading" class="loading-template-msg">
          Loading Enthalpy specification template...
        </div>
        ...
        <div *ngIf="!loading">
          ...
          <div *ngIf="propertyKeys.length === 0" class="no-props-msg">
            No detailed properties available for this Enthalpy specification.
          </div>
        </div>
      </div>
    </div>
...
export class ParameterSpecificationEnthalpyComponent extends BasePrimitiveComponent implements OnInit {
  ...
  override ngOnInit(): void {
    ...
    if (!hasProperties) {
      const cls = this.classname || this.structure?.classname || 'dataset:ParameterSpecificationEnthalpy';
      this.loading = true;
      this.ontologyService.getUITemplate(cls).subscribe({
        ...
        error: (err) => {
          console.error('[ParameterSpecificationEnthalpyComponent] Failed to load template for class: ' + cls, err);
          this.loading = false;
          this.cdr.detectChanges();
        }
      });
  ...
  mapGenericToSpecific(): void {
    ...
    // 1. Label Mapping
    if (!this.value['dataset:parameterlabelenthalpy']) {
      const labelVal = this.value['skos:prefLabel'] || this.value['dataset:parameterlabel'];
      if (labelVal) {
        this.value['dataset:parameterlabelenthalpy'] = labelVal;
      }
    }

    // 2. Units Mapping
    if (!this.value['dataset:molarenthalpyunits']) {
      ...
      if (srcUnit) {
        ...
        const unitclass = typeof srcUnit === 'string' ? 'dataset:UnitClassMolarEnthalpy' : (srcUnit['qudt:SystemOfQuantities'] || 'dataset:UnitClassMolarEnthalpy');
        unitspec = this.translateQudtUnit(unitspec);
        this.value['dataset:molarenthalpyunits'] = {
          'dataset:molarenthalpyunitclass': unitclass,
          'dataset:molarenthalpyunitspecification': unitspec
        };
      }
    } else {
      const specObj = this.value['dataset:molarenthalpyunits'];
      if (specObj && specObj['dataset:molarenthalpyunitspecification']) {
        specObj['dataset:molarenthalpyunitspecification'] = this.translateQudtUnit(specObj['dataset:molarenthalpyunitspecification']);
      }
    }

    // 3. Purpose/Concept Mapping
    if (!this.value['dataset:purposeconceptenthalpy']) {
      this.value['dataset:purposeconceptenthalpy'] = {
  ...
  getLabel(): string {
    if (this.value) {
      if (this.value['dataset:parameterlabelenthalpy']) return this.value['dataset:parameterlabelenthalpy'];
    ...
    return 'Enthalpy Specification';
  }

  getUnits(): string {
    const unitspec = this.value?.['dataset:molarenthalpyunits'] || this.value;
    const unitObj = unitspec?.['dataset:molarenthalpyunitspecification'] || unitspec?.['qudt:Unit'] || unitspec?.['dataset:units'];
    if (unitObj) {
  ...
  updateProperty(key: string, newValue: any): void {
    if (this.value) {
      ...
      if (key === 'dataset:parameterlabelenthalpy') {
        updatedValue['skos:prefLabel'] = newValue;
      } else if (key === 'dataset:molarenthalpyunits') {
        const unitspec = newValue?.['dataset:molarenthalpyunitspecification'];
        const unitclass = newValue?.['dataset:molarenthalpyunitclass'];
