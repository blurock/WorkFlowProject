export const environment = {
  production: true,
  apiBaseUrl: 'https://backend-data-manager-315685320181.europe-west1.run.app',
  datasetBackgroundUrl: 'https://dataset-background-services-315685320181.europe-west1.run.app',
  orchestratorUrl: 'https://backend-orchestrator-315685320181.europe-west1.run.app',
  firebase: {
    projectId: "blurock-database",
    appId: "1:315685320181:web:212000ed2f64b4c730419d",
    storageBucket: "blurock-database.firebasestorage.app",
    apiKey: "AIzaSyBFHXqA8MXdv-KbON_IU78BItS9KangM1Y",
    authDomain: "blurock-database.firebaseapp.com",
    messagingSenderId: "315685320181",
    measurementId: "REPLACE_WITH_REAL_MEASUREMENT_ID"
  },
  ketcherUrl: 'https://blurock-ketcher.web.app'
};
