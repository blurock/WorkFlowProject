...
public enum ServiceCollectionDatabaseAccess {
	...
	FindSetOfSymmetryElements {
		...
		public JsonObject process(JsonObject info) {
			...
			body.addElement("div").addText("Owner: " + symmetrytype);
			JsonArray arr = ExtractSetOfSymmetryDefinitionsFromDataset.databaseSymmetryDefinitions(owner,
			...
			if (arr != null) {
				...
				for (int i = 0; i < arr.size(); i++) {
					...
					JsonObject symdef = symmetry.get(ClassLabelConstants.JThermodynamicsSymmetryDefinition)
							.getAsJsonObject();
					...
					JsonObject struct = symmetry.get(ClassLabelConstants.JThermodynamics2DSpeciesStructure)
							.getAsJsonObject();
	...
	FindSymmetryElement {
		...
		public JsonObject process(JsonObject info) {
			...
			body.addElement("div").addText("dataset   : " + dataset);
			body.addElement("div").addText("Symmetry Type: " + symmetrytype);

			JsonObject symmetry = ExtractSetOfSymmetryDefinitionsFromDataset.databaseSingleSymmetryDefinition(
					maintainer,
	...
	FindSetOfMetaAtoms {
		...
		public JsonObject process(JsonObject info) {
			...
			JsonArray arr = FindMetaAtomDefinitionsInDatasetCollection.findMetaAtomDefinitions(maintainer, dataset,
					metaatomtype);
			if (arr != null) {
				body.addElement("div").addText("Read in " + arr.size() + " Symmetry elements");
				...
				for (int i = 0; i < arr.size(); i++) {
					...
					JsonObject metaatom = metaatomdefinition.get(ClassLabelConstants.JThermodynamicsMetaAtomInfo)
							.getAsJsonObject();
