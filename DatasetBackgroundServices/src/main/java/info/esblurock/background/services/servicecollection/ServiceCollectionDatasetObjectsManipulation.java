	...
	FindDatasetTransaction {
		...
		public JsonObject process(JsonObject json) {
			...
			try {
				...
				if (json.has(ClassLabelConstants.ActivityInformationRecord) && json.get(ClassLabelConstants.ActivityInformationRecord).isJsonObject()) {
				...
				} else if (info.has(ClassLabelConstants.TransactionEventType)) {
					type = info.get(ClassLabelConstants.TransactionEventType).getAsString();
				}
				...
				if (type == null) {
					return StandardResponse.standardErrorResponse(document, "Missing transaction event type parameter (type)", null);
				}
				
				JsonObject transaction = FindTransactions.findDatasetTransaction(info, type, onlyone);
				
				if (transaction != null) {
					JsonArray arr = new JsonArray();
					arr.add(transaction);
					response = StandardResponse.standardServiceResponse(document, "Success: FindDatasetTransaction", arr);
				} else {
					response = StandardResponse.standardErrorResponse(document, "No transaction found for type: " + type, null);
				}
			} catch (Exception ex) {
				response = StandardResponse.standardErrorResponse(document, "Error in FindDatasetTransaction: " + ex.toString(), null);
			}
			return response;
		}
