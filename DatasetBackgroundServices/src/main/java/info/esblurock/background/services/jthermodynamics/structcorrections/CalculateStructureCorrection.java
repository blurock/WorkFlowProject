...
public class CalculateStructureCorrection {
    ...
    public static JsonObject calculate(JsonObject info) {
        ...
        if (info.get(ClassLabelConstants.DatabaseCollectionRecordID) != null) {
            ...
            if (molecule != null) {
                JsonObject collectionresponse = FindJThermodynamics2DSubstructureThermodynamics
                        .findJThermodynamics2DSubstructureThermodynamics(maintainer, dataset, structuretype);
                if (collectionresponse.get(ClassLabelConstants.ServiceProcessSuccessful).getAsBoolean()) {
                    JsonArray structureCollection = collectionresponse.get(ClassLabelConstants.SimpleCatalogObject)
                            .getAsJsonArray();
    ...
    public static JsonObject calculate(JsonArray structureCollection, IAtomContainer molecule, JsonObject info,
            Document document) {
        JsonObject response = null;
        Element body = MessageConstructor.isolateBody(document);
        ...
        try {
            ...
            if (match != null) {
                JsonObject structure = match.get(ClassLabelConstants.JThermodynamics2DSpeciesStructure)
                        .getAsJsonObject();
                ...
                JsonObject contribution = JThermodynamicsFindSubstructure
                        .convertJThermodynamicsStructureToContribution(match, info);
                ...
                response = StandardResponse.standardErrorResponse(document,
                        "Error: No Corresponding structure found for radical", null);
            }
