...
public enum ServiceCollectionDatabaseCollectionSetAccess {
    ...
    GetListOfDatasetCollectionIDsSet {
        ...
        public JsonObject process(JsonObject json) {
            ...
            String maintainer = collrecordid.get(ClassLabelConstants.CatalogObjectOwner).getAsString();
    ...
    ReadInDatasetWithDatasetCollection {
        ...
        public JsonObject process(JsonObject json) {
            /*
             * Document document =
             * MessageConstructor.startDocument("ReadInDatasetWithDatasetCollection");
             * JsonObject idsset =
             * json.get(ClassLabelConstants.ChemConnectDatasetCollectionIDsSet).
             * getAsJsonObject();
             * String classname =
             * json.get(ClassLabelConstants.DatasetCollectionObjectType).getAsString();
             * String dataset =
             * idsset.get(ClassLabelConstants.DatasetCollectionsSetLabel).getAsString();
             * String maintainer =
             * idsset.get(ClassLabelConstants.CatalogObjectOwner).getAsString();
             * JsonObject criteria = null;
             * return ReadInDatasetWithDatasetCollectionProcess.process(maintainer, dataset,
             * classname, criteria, document);
             */
            ...
            String owner = idsset.get(ClassLabelConstants.CatalogObjectOwner).getAsString();
            ...
            if (identifier != null) {
                if (idsset.get(identifier) != null) {
                    ...
                    // String catalogtype =
                    // OntologyUtilityRoutines.exactlyOnePropertySingle(classname, "dcat:catalog");
                    ...
                    if (response.get(ClassLabelConstants.ServiceProcessSuccessful).getAsBoolean()) {
                        response = StandardResponse.standardServiceResponse(document,
                                "Success in reading dataset collection",
                                response.get(ClassLabelConstants.SimpleCatalogObject).getAsJsonArray());
                    } else {
