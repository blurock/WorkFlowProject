...
public class ProcessExamineDatabaseCollectionSetObject {
     ...
     *         info has:
     *         <ul>
     *         <li>DatasetCollectionSetRecordIDInfo The specification for the
     *         dataset collection set
     *         <ul>
     *         <li>DatasetCollectionsSetLabel The label of the collection set
     *         <li>CatalogObjectOwner The maintainer of the collection set
     *         </ul>
     *         <li>DatasetCollectionObjectType The specific classname of the dataset
     *         to convert
     *         </ul>
     * 
     */
    public static JsonObject process(JsonObject info) {
        ...
        if (set.size() > 0) {
            ...
            response = ReadInDatasetWithDatasetCollectionProcess.process(maintainer, dataset, classname, null,
                    document);
    ...
    private static void summaryOfSearchTerms(JsonArray labels, JsonObject terms,
            HashMap<String, HashSet<String>> summary) {
        for (int i = 0; i < labels.size(); i++) {
            String label = labels.get(i).getAsString();
