...
public class DatasetCollectionCreateSystemCollection {
    ...
    /**
     * This copies a collection set from a source to a destination, both specified
     * by a collection set ID label
     * 
     * @param event The event of the transaction
     * @param info  The information to make the copy
     * @return response
     * 
     *         The SourceCollectionOwner combined with the
     *         DatasetCollectionSetSourceLabel specifies the
     *         collection set to be used as the source
     *         The DestinationCollectionOwner combined with the
     *         DatasetCollectionSetDestinationLabel specifies the
     *         collection set to be used as the destination.
     * 
     *         Each set of catalog objects is specified with the DatasetVersion and
     *         the DatasetVersion given in the input
     *         (meaning they are all the same).
     * 
     *         A DescriptionTitle is used to give a description of the destination
     *         collection.
     * 
     *         TransactionID is the transaction id used to create the new collection
     * 
     *         Each set of the catalog objects in the source collection is copied
     *         into the location specified by the destination collection.
     * 
     * 
     */
    public static JsonObject copyCollectionSet(JsonObject event, JsonObject info, String objecttype) {
        ...
        String srcmaintainer = info.get(ClassLabelConstants.SourceCollectionOwner).getAsString();
        String destmaintainer = info.get(ClassLabelConstants.DestinationCollectionOwner).getAsString();
        ...
        body.addElement("div").addText("Collection Owner                : " + maintainer);
        body.addElement("div").addText("Source Owner                    : " + srcmaintainer);
        body.addElement("div").addText("Source Collection Name               : " + srccollectionlabel);
        body.addElement("div").addText("Destination Owner               : " + destmaintainer);
        ...
        collectionsetidinfo.addProperty(ClassLabelConstants.CatalogObjectOwner, srcmaintainer);
        ...
        if (datasetcollectionset != null) {
            ...
            systemcollectionset.addProperty(ClassLabelConstants.CatalogObjectOwner, destmaintainer);
            ...
            try {
                ...
                if (success) {
                    ...
                    response = StandardResponse.standardServiceResponse(document,
                            "Some (non-fatal) errors found, not all objects transferred", arr);
                }
            } catch (Exception e) {
    ...
    private static boolean copyDatasetElements(JsonObject systemcollectionset, String transactionid,
        ...
        if (datasetcollectionset.get(setlabel) != null) {
            ...
            MessageConstructor.combineBodyIntoDocument(document,
                    colresponse.get(ClassLabelConstants.ServiceResponseMessage).getAsString());
            if (colresponse.get(ClassLabelConstants.ServiceProcessSuccessful).getAsBoolean()) {
                ...
                for (JsonElement element : array) {
                    ...
                    try {
                        ...
                        body.addElement("div").addText("Catalog Object not written: "
                                + catalog.get(ClassLabelConstants.CatalogObjectKey).getAsString());
                        body.addElement("div").addText(e.getMessage());
                    }
                }
                JsonObject firestore = DatasetCollectionIDManagement.firebaseIDOfCollection(classname,
                        newspecification);
    ...
    private static JsonObject generateNewSpecification(String version, String collectionname, String destmaintainer) {
        ...
        newspec.addProperty(ClassLabelConstants.CatalogObjectOwner, destmaintainer);
