...
public class JThermodynamicsFindSubstructure {
    ...
    /**
     * @param species       The species in the
     *                      JThermodynamics2DSpeciesStructureDatabase form
     * @param substructures A JsonArray of
     *                      JThermodynamics2DSubstructureThermodynamicsDatabase
     *                      (read from the database)
     * @return the best (largest)
     *         JThermodynamics2DSubstructureThermodynamicsDatabase from the
     *         JsonArray
     * @throws CDKException This is thrown if there is a problem in converting the
     *                      JThermodynamics2DSubstructureThermodynamics to
     *                      IAtomContainer
     * 
     *                      This top level routine converts all
     *                      JThermodynamics2DSubstructureThermodynamicsDatabase and
     *                      JThermodynamics2DSpeciesStructureDatabase JsonObjects
     *                      to the JThermodynamicsAtomCountSet class (which extends
     *                      the AtomCounts) class.
     * 
     *                      The AtomCounts class is used to order the list of
     *                      structures.
     * 
     *                      GetSubstructureMatches is used to do the mappings
     *                      between the species and the substructure.
     * 
     ...
     * @param structureCollection The ArrayList of substructures to find in
     *                            speciessubstructure
     * @return The largest match from the array. If no match, result is null,
     * 
     *         This routine sorts the array of JThermodynamicsAtomCountSet from
     *         largest to smallest.
     *         In this order, each structure is tried. If there is a match, then
     *         that structure is returned.
     * 
     *         The mapping is tried only if the substructure is
     *         'strictlyLessThanOrEqual' (as defined by AtomCounts)
     *         than that of speciessubstructure
     * 
     *         GetSubstructureMatches is used to find the matches.
     * 
     *         A match is defined when one of the maps returned from the
     *         GetSubstructureMatches call
     *         has the same number of elements as the number of atoms in the
     *         substructure
    ...
    public static JsonObject convertJThermodynamicsStructureToContribution(JsonObject structure, JsonObject info) {
        ...
        JsonObject enthalpyparameter = thermo.get(ClassLabelConstants.ThermodynamicStandardEnthalpy).getAsJsonObject()
                .deepCopy();
        ParameterUtilities.changeParameterToNewSpecification(enthalpyparameter, info,
        ...
        enthalpyparameter.add(ClassLabelConstants.ParameterSpecificationEnthalpy, enthalpyspec);

        JsonObject entropyparameter = thermo.get(ClassLabelConstants.ThermodynamicStandardEntropy).getAsJsonObject()
                .deepCopy();
        ParameterUtilities.changeParameterToNewSpecification(entropyparameter, info,
        ...
        enthalpyparameter.add(ClassLabelConstants.ParameterSpecificationEntropy, entropyspec);
