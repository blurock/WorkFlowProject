#!/bin/csh
# ---------------------------------------------------------------------------
# Script: Read & Check ASCII Reaction Patterns
# ---------------------------------------------------------------------------
...
set CALLDIR    = `pwd`
set INFILEROOT = $1
set REFERENCE  = $REACTROOT/programs/inputs/ReadCheckReactionPatternsASCII.inp
...
$CHEMPROG read < read.prg >! $TEMPDIR/readcheckreactionpatternsascii.rawout
popd > /dev/null

echo "Read & check ASCII reaction patterns completed for $INFILEROOT."
