...
static DbaseMaster *InitializeDbaseMaster(CHAR *name) {
  ...
  CreateDbaseMaster(dbasemaster, 0, name, 0, 0);
  return (dbasemaster);
}
...
static void FillInExampleKeys(DataBaseInformation *dinfo) {
  ...
  CreateSetOfSearchKeyTypes(keytypes, 0, 0, 4, 4, 0);
  keytypes->NumberOfKeyTypes = 1;

  keyinfo = keytypes->KeyTypes;
  CreateSearchKeyInfo(keyinfo, 0, 0, 20, 10, 0, InsertExampleKey);
}

static INT InitializeExampleDatabase(DbaseMaster *dbmaster) {
  DataBaseInformation *info;

  CreateDbaseMaster(dbmaster, dbmaster->ID, dbmaster->Name, 1, 0);

  info = dbmaster->Databases;

  CreateDataBaseInformation(info, 0, "Example1", "example1", ".",
                            WriteBinExample1, ReadBinExample1, 0, 0,
                            AllocExample1, FreeExample1, NULL, NULL);
...
static DbaseKeyword *BuildExampleKey(Example1 *example) {
  ...
  CreateDbaseKeyword(key, example->ID, example->Name, sizeof(INT),
                     (CHAR *)&(example->ID));
...
static Example1 *BuildAnExampleElement(INT id, CHAR *name) {
  ...
  CreateExample1(example, id, name, num, 0);
