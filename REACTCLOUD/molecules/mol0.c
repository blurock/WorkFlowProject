...
extern void PrintPrettyMoleculeElectronic(CHAR *prefix, FILE *file,
  ...
  PrintPrettyAtomElectronic(prefix, file, molecule->Atoms, molecule->NAtoms);
...
extern void PrintPrettyAtomElectronic(CHAR *prefix, FILE *file,
  ...
  atom = atomstart;
  LOOPi(NAtoms) {
    fprintf(file, "%s:%11.3d:%11.3f:%11.3f:%10d", string, atom->AtomicNumber,
            atom->Charge, atom->NElectrons, atom->NOccupied);
    LOOPj(atom->NOccupied) fprintf(file, "%5.3f ", atom->Shells[j]);
    fprintf(file, "\n");
    atom++;
  }
  fprintf(file, "~\n");
  ...
  atom = atomstart;
  LOOPi(NAtoms) {
    fprintf(file, "%s:%11.6f%11.6f%11.6f%11.6f%11.6f%11.6f\n", string,
            atom->CovalentRadius, atom->Electronegativity, atom->Screening,
            atom->NuclearCharge, atom->ResidualCharge, atom->Beta);
    atom++;
  }
  fprintf(file, "~\n");
}
...
extern void PrintPrettySetOfResonances(CHAR *prefix, FILE *file,
  ...
  LOOPi(sets->NumberOfSets) {
    ...
    atm = set->Atoms;
    LOOPj(set->NumberOfAtoms) fprintf(file, "%3d", *atm++);
    fprintf(file, "\n");
    set++;
  }
}
...
**
*/
extern MoleculeInfo *FindMoleculeFromString(CHAR *molname, MoleculeSet *set) {
  INT id, flag;
  MoleculeInfo *mol;
...
extern MoleculeSet *MergeMoleculeSets(MoleculeSet *molset1,
  ...
  CreateMoleculeSet(newset, molset1->ID, molset1->Name, size, 0, 0, 0);
