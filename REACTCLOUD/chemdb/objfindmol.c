...
**
*/
extern INT PutMoleculeInDatabaseClass(MoleculeInfo *molecule, INT classid,
                                      BindStructure *bind) {
  DataBaseInformation *dinfo;
  ObjectClassification *classification;
  ...
  if (id == 0) {
    dinfo = GetDataBaseInfoFromID(master->DatabaseInfo,
                                  classification->Description->Database);
    class = FindOrCreateObjectIDClass(molecule, classification->Description,
    ...
    printf("Found in database: %d (Equivalent to molecule: %s)\n", id,
           molecule->Name);
    molecule->ID = id;
  }
  return (molecule->ID);
...
extern INT InitializeMoleculeClassifications(BindStructure *bind) {
  ...
  classification =
      FindClassification(MOLECULE_DATABASE, LOCAL_CLASSIFICATIONS, bind);
  FreeObjectIDTreeNode(classification->TreeOfObjects);
  CreateObjectIDTreeNode(classification->TreeOfObjects, classification->ID,
                         classification->Name, 0, 0, 0);
  ret = ReadKeyIntoMemory(DB_ID_SEARCH, dinfo);
  if (ret == SYSTEM_NORMAL_RETURN)
    ReadKeyIntoMemory(DB_NAME_SEARCH, dinfo);
  molinfo = bindmols->Molecules;
  LOOPi(bindmols->NumberOfMolecules) {
    id = DetermineObjectID((VOID)molinfo, MOLECULE_DATABASE,
                           LOCAL_CLASSIFICATIONS, bind);
    if (id == 0)
...
**
*/
extern ListOfTreeLevelDescriptions *
InitializeMoleculeTreeDescription(INT id, CHAR *name) {
  ListOfTreeLevelDescriptions *descr;
  TreeLevelDescription *level;

  descr = AllocateListOfTreeLevelDescriptions;
  CreateListOfTreeLevelDescriptions(descr, id, name, 4, 0,
                                    BIND_CURRENT_MOLECULES, id, DB_ID_SEARCH);

  level = descr->Levels;
  CreateTreeLevelDescription(level, 0, "Number of Bonds", MOLID_NUM_BONDS,
                             MOLID_MAX_NUM_BONDS);
  level->LevelIDFunc = DetermineNumberOfBonds;

  level++;
  CreateTreeLevelDescription(level, 1, "Number of Atoms", MOLID_NUM_ATOMS,
                             MOLID_MAX_NUM_ATOMS);
  level->LevelIDFunc = DetermineNumberOfAtoms;

  level++;
  CreateTreeLevelDescription(level, 2, "Number of Carbons", MOLID_NUM_CARBONS,
                             MOLID_MAX_NUM_CARBONS);
  level->LevelIDFunc = DetermineNumberOfCarbons;

  level++;
  CreateTreeLevelDescription(level, 3, "Structure Type", MOLID_STRUCTURE,
                             MOLID_MAX_STRUCTURE);
...
extern VOID FindMoleculeByKey(DbaseKeyword *key, VOID setofelements) {
  ...
  molinfo =
      FindMoleculeInMoleculeSetByName(key->Name, (MoleculeSet *)setofelements);
...
extern INT MoleculeEquivalence(VOID mol1, VOID mol2) {
  ...
  grph1 = GraphFromMolFile((MoleculeInfo *)mol1, VALENCE_STANDARD);
  grph2 = GraphFromMolFile((MoleculeInfo *)mol2, VALENCE_STANDARD);
...
extern INT SubStructureEquivalence(VOID mol1, VOID mol2) {
  ...
  grph1 = GraphFromMolFile((MoleculeInfo *)mol1, VALENCE_ATOMIC_NUMBER);
  grph2 = GraphFromMolFile((MoleculeInfo *)mol2, VALENCE_ATOMIC_NUMBER);

  pairsset = BasicSubGraph(grph1, grph2);
  printf("Equivalence: %10s %10s %5d\n", ((MoleculeInfo *)mol1)->Name,
         ((MoleculeInfo *)mol2)->Name, pairsset->NumberOfSets);
