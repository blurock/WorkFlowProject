...
extern INT StoreMoleculeSetToDatabase(MoleculeSet *set, INT dbflag,
  ...
  if (set == NULL || set->Molecules == NULL || set->NumberOfMolecules <= 0) {
    printf(
        "StoreMoleculeSetToDatabase: No molecules in current set to store.\n");
    fflush(stdout);
...
**     A list of MolFiles are given in a file.  Each one is opened and read and
*put
...
**    ret: SYSTEM_NORMAL_RETURN, SYSTEM_ERROR_RETURN
**
**    This is the main work routine to read in a set of molecules or
*substructures.
...
static INT DBReadMolSubsFromList(BindStructure *bind, INT dbflag) {
  ...
  while (line != 0) {
    ...
    file = MasterOpenReadFile(dir, line, MOL_FILE_SUFFIX, IGNORE,
...
**    The database is looped through using FetchFirstElement and
*FetchNextElement.
**
**  REMARKS
**
...
*/
extern MoleculeSet *
DetermineMoleculeSetFromASCII(ReadInMoleculeSet *asciimolset, INT dbflag,
                              BindStructure *bind) {
  DataBaseInformation *dinfo;
  MoleculeSet *molset;
  MoleculeInfo *mol;
  ...
  CreateSetOfMolFileMetaAtoms(molset->MetaAtoms, asciimolset->ID,
                              asciimolset->Name, 0, 0);
