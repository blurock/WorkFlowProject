...
extern INT StoreCurrentBensonTables(BindStructure *bind) {
  ...
  if (bentables != 0) {
    ...
    LOOPi(bentables->NumberOfTables) {
      CreateBensonTables(btable, single->ID, single->Name, single, 0,
...
extern INT StoreCurrentBensonTree(BindStructure *bind) {
  ...
  CreateBensonTables(btable, bentree->ID, bentree->Name, 0, bentree, 0);
...
extern INT FillInChemkinInCurrent(BindStructure *bind) {
  ...
  dinfo =
      GetDataBaseInfoFromID(dbmaster->DatabaseInfo, CHEMKIN_THERMO_DATABASE);
  ...
  LOOPi(molset->NumberOfMolecules) {
    if (mol->ID >= 0) {
      values = FindValuesFromType(ALTERNATIVE_NAMES_PROPERTY, mol->Properties);
      if (values != NULL) {
        ...
        if (value != 0) {
          ...
          CreateDbaseKeyword(key, CHEMKIN_THERMO_DATABASE, value->Value,
                             (INT)strlen(value->Value) + 1, value->Value);
...
extern INT PrintChemkinNameCorrespondences(BindStructure *bind) {
  ...
  file =
      OpenWriteFileFromCurrent("ChemkinDirectory", "RootChemkinName", "lst",
...
extern DbaseKeyword *ComputeBensonTreeKey(CHAR *name) {
  ...
  key = AllocateDbaseKeyword;
  CreateDbaseKeyword(key, BENSON_TABLES_DATABASE, name, (INT)strlen(name) + 1,
                     name);
  return (key);
}
...
*/
extern DbaseKeyword *
ComputeBensonTable2ndOrderKey(BensonSecondOrderTable *thermo) {
  DbaseKeyword *key;

  key = AllocateDbaseKeyword;
  CreateDbaseKeyword(key, BENSON_TABLES_DATABASE, thermo->Name,
                     (INT)strlen(thermo->Name) + 1, thermo->Name);
  return (key);
}
...
extern INT ReadBensonTablesFromList(BindStructure *bind) {
  ...
  file = OpenReadFileFromCurrent("TablesDataDirectory", "TablesDataBensonRoot",
                                 MOL_FILE_LIST_SUFFIX, IGNORE,
                                 "List of Molfiles", commandmaster);
  dir = GetCurrentStringArgument("TablesDataDirectory", commandmaster);
  root = GetCurrentStringArgument("TablesDataBensonRoot", commandmaster);
  printf("REACT Directory: %s, Root File Name: %s.%s\n", dir, root,
  ...
  if (file != 0) {
    ...
    CreateSetOfBenson2ndOrderTables(set, master->ID, master->Name, maxnum, 0);
    ...
    atomtrans = tables->AtomTranslations = AllocateSetOfBensonAtomTranslationPairs;
    CreateSetOfBensonAtomTranslationPairs(atomtrans, master->ID, line, 0, 0);
    ...
    while (line != 0 && count < maxnum) {
      ...
      CreateDbaseKeyword(key, 0, line, (INT)strlen(line) + 1, line);
      printf("Fetch: %s\n", line);
      INT ret = FetchElement((VOID)btable, key, dinfo);
      printf("Fetch returned: %d\n", ret);
...
static void MergeAtomTranslationTables(SetOfBensonAtomTranslationPairs *total,
  ...
  CreateSetOfBensonAtomTranslationPairs(new, total->ID, total->Name, totalnum,
                                        0);
    ...
    while (done == 0 && count < new->NumberOfPairs) {
      if (!strcmp(next->AtomName, oldpair->AtomName)) {
        ...
        if (next->Valence != oldpair->Valence) {
          string = AllocateString(LINELENGTH);
          sprintf(string,
                  "For Atom %s two valence values %d %d (choosing first)\n",
...
extern INT DBReadInBensonTree(BindStructure *bind) {
  ...
  btable = AllocateBensonTables;
  ret = FetchElement((VOID)btable, key, dinfo);
