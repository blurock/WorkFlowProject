...
**     that are available.  This means setting up the SetOfSearchKeyTypes
*structure and
...
extern INT DBResetMolSubsSearchKeys(BindStructure *bind, INT dbflag) {
  ...
  CreateSetOfSearchKeyTypes(keytypes, 0, 0, 5, 2, 0);
  keytypes->NumberOfKeyTypes = 2;

  keyinfo = keytypes->KeyTypes;
  CreateSearchKeyInfo(keyinfo, DB_ID_SEARCH, "Search by ID", 20, 10, 0,
                      InsertMolIDKey);
  keyinfo = keytypes->KeyTypes + 1;
  CreateSearchKeyInfo(keyinfo, DB_NAME_SEARCH, "Search by Name", 20, 10, 0,
                      InsertMolNameKey);
...
extern INT DBReadListMolSubFromKeys(BindStructure *bind, INT dbflag) {
  ...
  file = OpenReadFileFromCurrent("DBDataDirectory", "DBDataMolRoot",
                                 ...
                                 "List of Molecules", commandmaster);
  ...
  if (file != 0) {
    ...
    while (line != 0) {
      ...
      /*	       if(molset->NumberOfMolecules >=
         molset->NumberMoleculesAllocated) IncrementMoleculeSet(molset);
      */
      line = NextNonBlankLine(file, string);
    }
    fclose(file);
...
**    ret: SYSTEM_NORMAL_RETURN, SYSTEM_ERROR_RETURN
**
**  The MoleculeSet structure is expanded by the MoleculeIncrement (to allow
*more
...
extern INT DBFindMolSubFromString(CHAR *name, MoleculeInfo *mol,
  ...
  if (dinfo == NULL || mol == NULL || name == NULL) return SYSTEM_ERROR_RETURN;
...
extern INT readMoleculesFromListOfNames(INT nummols, CHAR *names[],
  ...
  if (dbflag == PATTERN_DATABASE || dbflag == SUBSTRUCTURE_DATABASE) {
    ...
    bind_target = BIND_CURRENT_SUBSTRUCTURES;
  } else {
    dbflag = MOLECULE_DATABASE;
    bind_target = BIND_CURRENT_MOLECULES;
  }
  ...
  dinfo = (master != NULL && master->DatabaseInfo != NULL)
              ? GetDataBaseInfoFromID(master->DatabaseInfo, dbflag)
              : NULL;
  molset = GetBoundStructure(bind, bind_target);
  FreeMoleculeSet(molset);
  CreateMoleculeSet(molset, bind_target, "Current Molecules",
                    nummols, 0, 0, 0);

  molset->NumberOfMolecules = nummols;
  molset->PropertyTypes =
      InitializeMolecularPropertyTypes(molset->ID, molset->Name);
  molinfo = molset->Molecules;
  for (i = 0; i < nummols; i++) {
    name = names[i];
    ret = DBFindMolSubFromString(name, molinfo, dinfo);
    if (ret == SYSTEM_NORMAL_RETURN) {
      printf("ID = %5d Name = %s\n", molinfo->ID, molinfo->Name);
    }
    molinfo++; /* Advance to next molecule struct */
  }
  return SYSTEM_NORMAL_RETURN;
}
...
**    numReactants: Number of reactant molecule IDs in names array
**    numProducts: Number of product molecule IDs in names array
**    names: Array of molecule names/IDs (reactants first, then products)
**    out: Output file stream (e.g. stdout or file)
**    dbflag: Database flag (REACTION_DATABASE vs PATTERN_DATABASE)
**    bind: Bind structure
**
**  REMARKS
**    Calls readMoleculesFromListOfNames to fetch reactant and product
**    molecules into BIND_CURRENT_MOLECULES/SUBSTRUCTURES, prints the RXN preamble,
...
extern INT PrintRXNFromListOfNames(CHAR *rxnName, INT numReactants,
  ...
  /* Read reactant and product molecules into BIND_CURRENT_MOLECULES/SUBSTRUCTURES */
  ret = readMoleculesFromListOfNames(nummols, names, dbflag, bind);
  if (ret != SYSTEM_NORMAL_RETURN) {
    return ret;
  }
