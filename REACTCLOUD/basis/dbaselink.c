...
{
    INT len;
    if(json_output == 0 || *json_output == 0) return;
    if(string != 0) {
        len = sprintf(*json_output, "\"%s\" ", string);
        *json_output += len;
    } else {
        len = sprintf(*json_output, "null ");
        *json_output += len;
    }
}
...
extern INT ReadJSONSTRING(CHAR *string, CHAR **json_input)
{
    ...
    CHAR *rest;
    if(json_input == 0 || *json_input == 0) return NO_STRUCTURE_CODE;
    rest = IsolateNextWord(*json_input, buffer, ' ', LINELENGTH);
    if(buffer[0] == '\0' || strcmp(buffer, "null") == 0) {
        if(string) string[0] = '\0';
        *json_input = rest;
        return NO_STRUCTURE_CODE;
    }
    if(string) strcpy(string, buffer);
    *json_input = rest;
    return STRUCTURE_READ;
}
...
extern INT ReadJSONNAME(CHAR **string, CHAR **json_input)
{
    ...
    if(string) *string = CopyString(buffer);
    *json_input = rest;
    return STRUCTURE_READ;
}
...
{
    INT len;
    if(json_output == 0 || *json_output == 0 || element == 0) return 0;
    if(size == sizeof(INT)) {
        len = sprintf(*json_output, "%d ", *(INT *)element);
    } else if(size == sizeof(FLOAT)) {
        len = sprintf(*json_output, "%f ", *(FLOAT *)element);
    } else if(size == sizeof(CHAR)) {
        len = sprintf(*json_output, "%c ", *(CHAR *)element);
    } else {
        len = sprintf(*json_output, "%d ", *(INT *)element);
    }
    *json_output += len;
...
extern INT ReadJSON(void *element, INT size, CHAR **json_input)
{
    CHAR buffer[LINELENGTH];
    CHAR *rest;
    if(json_input == 0 || *json_input == 0 || element == 0) return NO_STRUCTURE_CODE;
    rest = IsolateNextWord(*json_input, buffer, ' ', LINELENGTH);
    if(buffer[0] == '\0') {
        *json_input = rest;
        return NO_STRUCTURE_CODE;
    ...
    if(size == sizeof(INT)) {
        *(INT *)element = ConvertStringToInteger(buffer);
    } else if(size == sizeof(FLOAT)) {
        *(FLOAT *)element = ConvertStringToFloat(buffer);
    } else if(size == sizeof(CHAR)) {
