...
extern CHAR *ReadInPropertyInfoSDF(FILE *file, CHAR *line,
                                   ASCIIProperties *propset) {
  CHAR *name;
  INT len;
  ASCIIProperty *prop;
  ...
  CreateASCIIProperties(propset, 0, 0, MAX_ASCII_PROPERTIES, 0);
  ...
  while (*line == '>' && propset->NumberOfProperties < MAX_ASCII_PROPERTIES) {
    ...
    if (len > 0) {
      ...
      *(name + len) = '\0';
      ...
      NextNonBlankLine(file, line);
      CreateASCIIProperty(prop, propset->NumberOfProperties, name, line);
...
extern void FillInASCIIValue(ASCIIProperties *asciipropset,
                             ...
                             SetOfPropertyTypes *types, BindStructure *bind) {
  ...
  if (asciipropset != 0) {
    ...
    LOOPi(asciipropset->NumberOfProperties) {
      FillInPropertyValueFromStringFromName(
          asciiprop->PropertyValue, asciiprop->Name, propset, types, bind);
      asciiprop++;
    }
  }
}
...
extern CHAR *FillInProperties(FILE *file, ASCIIProperties *props, CHAR *string) {
  CHAR *line, *word, *rest, *valbuf, *nextline;
  INT done;
  ASCIIProperty *prop;

  word = AllocateString(PRINT_BUFFER_LENGTH);
  valbuf = AllocateString(20 * PRINT_BUFFER_LENGTH);

  CreateASCIIProperties(props, 0, 0, MAX_ASCII_PROPERTIES, 0);
    ...
    if (line != 0) {
      ...
      if (strncmp(line, "$$$$", 4) == 0) {
        ...
        if (rest != 0 && *rest != '\0') {
          ...
          valbuf[0] = '\0';
          nextline = NextNonBlankLine(file, string);
          while (nextline != 0) {
            nextline = EliminateLeadingBlanks(nextline);
            if (*nextline == '-' || *nextline == '>' || strncmp(nextline, "$$$$", 4) == 0)
              break;
            if (valbuf[0] != '\0')
              strcat(valbuf, " ");
            strcat(valbuf, nextline);
            nextline = NextNonBlankLine(file, string);
          }
          CreateASCIIProperty(prop, props->NumberOfProperties, word, valbuf);
          printf("[FillInProperties] Read multi-line property '%s': '%s'\n", word, valbuf);
          prop++;
          props->NumberOfProperties += 1;
          line = nextline;
        }
        ...
        if (*rest == '<')
          rest++;
        rest = IsolateNextWord(rest, word, '>', PRINT_BUFFER_LENGTH);
        if (word[0] == '\0')
          IsolateNextWord(rest, word, BLANK, PRINT_BUFFER_LENGTH);
        EliminateBlanks(word);

        nextline = NextNonBlankLine(file, string);
        if (nextline != 0 && strncmp(nextline, "$$$$", 4) != 0) {
          CreateASCIIProperty(prop, props->NumberOfProperties, word, nextline);
          printf("[FillInProperties] Read tag property '%s': '%s'\n", word, nextline);
          prop++;
          props->NumberOfProperties += 1;
          line = NULL;
        } else {
...
**
*/
extern ASCIIProperties *ReadInListOfNamesAndProps(FILE *file, INT max,
                                                  CHAR delimitor) {
...
**
*/
extern ASCIIProperties *ReadListFromLine(CHAR *line, INT max, CHAR setdelimitor,
                                         CHAR namedelimitor) {
  ASCIIProperties *aprops;
  ASCIIProperty *prop;
