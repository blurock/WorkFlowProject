...
/*P  . . . PROTOTYPES  . . . . . . . . . . . . . . . . . . . . . . . . . . .
 */
static INT FillInCentralAtom(CHAR *line, BensonSecondOrder *value,
                             SetOfBensonAtomTranslationPairs *translation);
static INT FillInConnections(CHAR *line, BensonSecondOrder *value,
                             SetOfBensonAtomTranslationPairs *translation);
static INT ReadInBensonAtomTranslationSingleLine(CHAR *line, INT count,
...
extern INT BindReadIn2ndOrdBensonTable(BindStructure *bind) {
  ...
  file = OpenReadFileFromCurrent(
      "TablesDataDirectory", "TablesDataBensonRoot", BENSON_2NDORDER_SUFFIX,
  ...
  if (file != 0) {
    ...
    CreateSetOfBenson2ndOrderTables(bentables, bind->ID, bind->Name, 1, 0);
...
*/
extern INT
ReadIn2ndOrderBensonTable(FILE *file, BensonSecondOrderTable *table,
                          SetOfBensonAtomTranslationPairs *translation) {
  ...
  if (line != 0) {
    ...
    if (line != 0) {
      ...
      CreateBensonSecondOrderTable(table, 0, name, MAX_NUMBER_BENSON_VALUES, 0,
                                   0, 0, 0);
...
*/
extern INT
ReadInBensonSingleLine(CHAR *line, INT count, BensonSecondOrder *value,
                       SetOfBensonAtomTranslationPairs *translation) {
  INT ret;

  CreateBensonSecondOrder(value, count, 0, 0, MAX_BENSON_BOUND, 0, 0);
...
static INT FillInConnections(CHAR *line, BensonSecondOrder *value,
  ...
  while (line != 0 && value->NumberOfConnections < MAX_BENSON_BOUND) {
    ...
    if (strlen(word) != 0) {
      CreateBensonAtomConnections(connection, value->NumberOfConnections, word,
                                  0, 0);
      ...
      printf("Connection: %s (%d %d)\n", translation->Name, connection->Valence,
...
*/
extern INT FillInThermoValues(CHAR *line, BensonSecondThermoTable *thermo) {
  CHAR *rest, *word;
  FLOAT *flt;
  INT count;
  ...
  rest = line;
  word = AllocateString(25);
  CreateBensonSecondThermoTable(thermo, 0, 0, (FLOAT)0.0, (FLOAT)0.0, 7, 0);
...
extern INT PrintOutBensonTables(BindStructure *bind) {
  ...
  if (tables->SecondOrder != 0) {
    file = OpenWriteFileFromCurrent(
        "TablesOutputDirectory", "TablesDataBensonRoot", "out", IGNORE,
        "Chemkin Thermodynamic Data", commandmaster);
    if (file != 0)
...
**
*/
extern INT WriteOutStandardSecondOrderBensonTableSet(
    FILE *file, SetOfBenson2ndOrderTables *set,
    SetOfBensonAtomTranslationPairs *translation) {
...
/*F string =
 *StringSecondOrderBensonTableThermo(instring,length,thermo,translation)
 **
 **  DESCRIPTION
 **
 **
 ...
 **
 */
extern CHAR *StringSecondOrderBensonTableThermo(
    CHAR *string, INT *length, BensonSecondOrderTable *thermo,
    SetOfBensonAtomTranslationPairs *translation) {
  CHAR *line;
  FLOAT *temp;
  BensonSecondOrder *value;
...
*/
extern CHAR *
BensonSecondOrderLine(CHAR *in, INT *length, BensonSecondOrder *value,
                      SetOfBensonAtomTranslationPairs *translation) {
  CHAR *trans, *line;
  INT i;
...
*/
extern CHAR *
PrintStringBensonSecondThermoTable(CHAR *in, INT *length, CHAR *prefix,
...
extern INT ReadInBensonTranslationTables(BindStructure *bind) {
  ...
  FILE *file;
  INT ret = SYSTEM_ERROR_RETURN;

  tables = GetBoundStructure(bind, BIND_THERMO_TABLES);
  commandmaster = GetBoundStructure(bind, BIND_COMMANDMASTER);

  file = OpenReadFileFromCurrent("TablesDataDirectory",
                                 "TranslationDataBensonRoot",
                                 BENSON_TRANSLATE_SUFFIX, IGNORE,
                                 "Benson Translation Tables", commandmaster);

  if (file != 0) {
    if (tables->AtomTranslations != 0) {
      FreeSetOfBensonAtomTranslationPairs(tables->AtomTranslations);
      Free(tables->AtomTranslations);
      tables->AtomTranslations = 0;
    }
...
*/
extern INT
ReadInBensonTranslationPairs(FILE *file,
                             SetOfBensonAtomTranslationPairs *translation) {
  CHAR *string, *line, *name;
  INT count, ret;
  BensonTranslationPair *value;
...
static INT ReadInBensonAtomTranslationSingleLine(CHAR *line, INT count,
  ...
  CreateBensonTranslationPair(value, count, 0, 0, word);
  rest = IsolateNextWord(rest, word, BLANK, 20);
...
extern INT PrintBensonAtomTranslation(BindStructure *bind) {
  ...
  if (tables != 0 && tables->AtomTranslations != 0) {
    file = OpenWriteFileFromCurrent(
        "TablesOutputDirectory", "TranslationDataBensonRoot", "out", IGNORE,
        "Benson Atom Translation Table", commandmaster);
    if (file != 0) {
      orig_string = AllocateString(PRINT_BUFFER_LENGTH);
      string = orig_string;
      length = PRINT_BUFFER_LENGTH;
...
*/
extern CHAR *
StringBensonTranslationTable(CHAR *in, INT *length,
  ...
  if (pairs != 0) {
    if (pairs->Name != 0)
      ...
      in = PrintStringString(in, length, "Atom Translation Table");
    in = PrintStringString(in, length, "\n");
...
extern INT TranslateAtomSymbol(CHAR *string,
  ...
  if (translation == 0 || translation->Pairs == 0 || string == 0)
    return 0;
...
*/
extern CHAR *
TranslateValenceToString(INT valence,
                         SetOfBensonAtomTranslationPairs *translation) {
  BensonTranslationPair *pair;
  INT count;
  CHAR *done;

  if (translation == 0 || translation->Pairs == 0)
    return "??";
